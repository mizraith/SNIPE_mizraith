...
protocol that allows you to do the following:
    ...
    (3) Ping the device 
            Blinks the Arduino so you can figure out which USB device
            you are talking to.

Hardware Recommendation:
    SNIPE is best run on an Arduino with a USB-serial port built in, preferably
    one of Arduino's that uses an FTDI chip.  Here's why -- after programming
    your Arduino with SNIPE, you can use FTDI's free FTDI_PROG utility to 
    edit the USB device descriptors and make your device appear as a general
...
Serial Protocol:
    ...
    Where "c" is the START CHARACTER, e.g, '>' or '!' or '
    ...
    may have been added.
    Note that message token order is not typically guaranteed.
    
    Note that it should be possible to send two or more command tokens
    in a given message.  This will result in multiple responses and 
    commands are typically processed in the order they are received.
    However, given I2C commands there is some operation processing
    precedence (see Commmands).   If you send a 
    transaction ID (TID), then all the responses will return that same TID.
    ...
    VALUES (escaping):
        If a value might contain <CR> or <LF> as a byte (for instance when
        sending a byte stream to an I2C, then consider sending the value
        as a hex, with units as "HEX". e.g.:    <command>:0A12FFDE:HEX
        Some commands will default to interpreting the input string
        as hex.

    UNITS:
        Units are more of a convenience subtoken (a GUI may use them to
        label fields for instance), but may be used for parsing commands.
    ...
    IDENTIFIERS:
        ...
        Identifiers are returns in all caps, but should be considered
        case -insensitive-.
        ...
        [A0], A1, A2, A3
              ...
              units:        ARB
              example:      A0:254:ARB
              Note:  A0 is the recommended standard analog input.
        ...
        D1, D2, D3, D4, D5, [D6]    
              ...
              units:        BIN
              example:      D6:0:BIN
              Note:     D6 is the recommended standard digital output since
              it is not used for much on the Arduino
              
        SID   description:  Station ID  
              value range:  string value set by user, can NOT contain spaces
              example:      SID:My_Uno_3
              
        TID   description:  Transaction ID.  If the command contains a TID,
                            then SNIPE will return a response with the same TID.
              value range:  String, less than 8 characters please
              example:      command:  >A0:? TID:1a35
                            response: @A0:245 TID:1a35
                        
        DESC  description:  Description of our device
    ...
    COMMANDS:
        ...
        TID   description:  Transaction IDs can be included in messages. They
                            will be echoed back in the associated resopnse. The
                            intent is to support synchronizing commands
                            and responses.
        ...
        BLINK description:   Blinks the Arduino onboard LED (D13 typically)
              input argument:  0:5000   Number of milliseconds to blink for
              example:       command:  >BLINK:5000
                             response: @BLINK:5000
        
        
        I2A     description:  I2C target chip address
                input argument:  ?  for last setting
                                 0:127   (8 bit value as a decimal string)
                example:    command:  >I2A:125
                            response: @I2A:125
                            command:  >I2A:?
                            response: @I2A:125
        
        I2S     description:  I2C target setting (byte or register address)
                input argument:  ?  for last setting
                                 0:255 (8 bit value as a decimal string)
                example:    command:  >I2S:17
                            response: @I2S:17
                            command:  >I2S:?
                            response: @I2S:17
                        
        
        I2B      description:  I2C number of bytes to send or receive
                 input argument: ? for last setting
                                 0:16  (number of bytes as a decimal string)
                 example:   command:  >I2B:2
                            response: @I2B:2
                            command:  >I2B:?
                            response: @I2B:2
        
        I2W      description:  I2C write to currently set address, setting 
                               with byte count.
                 ...
                 NOTE:         THIS will be processed after all other items in the
                 message, but before an I2R
                 example:   command:  >I2A:110 I2S:23 I2B:2 I2W:0xA0F3 TID:14397
                            ...
                            response: !I2W:TOO_MANY_BYTES TID:14399
                            command:  >I2B:?
                            response: @I2B:2
                            command:  >I2B:3
                            response: @I2B:3
        ...
        I2R      description:  I2C read for currently set address, setting, bytes
                 input argument:  ?    <grammatically required>
                 NOTE:    THIS will be processed after all other items in the
                           message, and after I2W
                 example:   command:  >I2A:110 I2S:23 I2B:2 I2R:? TID:14397
                            response: @I2A:110 I2S:23 I2B:2 I2R:0xA0F3 TID:14397
                            command:  >I2R:?      (re-read from same chip/addr)
                            response: @I2R:0xA0F3        
                            command:  >I2A:1 I2R:? TID:14399
                            response  !I2A:1 I2R:READ_ERROR      (inccorrect address?)
         
        I2F     description:  Get a listing of chips on the I2C bus
     ...
     SENDING I2C COMMANDS -- MORE DETAIL:
        ...
        And theoretically you can mess with the order as well because Writes
        are processed near last and reads are processed dead last.  The 
        following would first set the address, setting, bytes, the write
